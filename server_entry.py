...
server_entry.py
---------------
PyInstaller entry point for the Omni-Striker AI backend server.
...
if getattr(sys, "frozen", False):
    ...
    os.environ.setdefault("OMNISTR_DATA_DIR", str(_EXE_DIR / "data"))
else:
    _EXE_DIR = Path(__file__).parent
    sys.path.insert(0, str(_EXE_DIR))

# ── Brain store data-dir override ─────────────────────────────────────────────
_data_dir = Path(os.environ.get("OMNISTR_DATA_DIR", str(_EXE_DIR / "weaponized_ai" / "brain")))
...
if __name__ == "__main__":
    ...
    log.info("Omni-Striker AI backend starting on http://127.0.0.1:8000")
    ...
    try:
        ...
        log.critical("Omni-Striker backend crashed: %s", exc, exc_info=True)
